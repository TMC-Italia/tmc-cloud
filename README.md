...
TMC Cloud is a self-hosted cloud platform designed to empower users to run their own cloud services using repurposed hardware, such as old laptops. It provides a user-friendly interface for managing containers and other cloud resources, making cloud computing accessible without reliance on third-party providers. This project is aimed at BMs and Employeneurs looking to leverage a private cloud infrastructure.

- **Timeline**: Q2 2025 completion target

## Project Overview

This project transforms legacy company PCs into a powerful on-premises private cloud infrastructure using Kubernetes and GitLab. The solution provides a scalable, fault-tolerant environment that supports essential applications and continuous integration processes while optimizing cost-efficiency and sustainability.

### Key Features

- **Kubernetes Orchestration**: Container orchestration with high availability
- **GitLab Integration**: Complete DevOps pipeline with CI/CD
...
Kubernetes Ingress Gateway  
    ↓  
┌─────────────────┬─────────────────┬─────────────────┐  
│ Application     │ GitLab Server   │ GitLab Runner   │  
│ Pods            │ (Repos, DB)     │ Pods (CI/CD)    │  
└─────────────────┴─────────────────┴─────────────────┘  
    ↓  
...
|Worker Node 2|1|4 cores|8 GB|200 GB SSD|GitLab Runner + CI/CD|
...
Installs Tailscale and guides you through authentication to securely connect all nodes via a mesh VPN.
You will be prompted to authenticate in your browser.

    setup-cloudflare.sh

Installs Cloudflare’s cloudflared client and guides you through authentication to expose internal services securely to the public internet.
You will be prompted to authenticate and can then create tunnels for your services.

Example usage
After running these scripts, follow the on-screen instructions to complete setup and connect your nodes/services.
...
├── README.md
├── docs/
│   ├── [A1] - Hosting and Cloud project
│   ├── [A2] - Architecture
│   ├── [A4] - Remote Access Setup
├── scripts/
...
- **Use Case**: External access to GitLab and applications
...
- **Storage**: NFS, MinIO, Ceph/MicroCeph (optional)

### GitLab Configuration

- **GitLab Server**: Complete DevOps platform
...
- **GitLab Metrics**: Pipeline success rates, repository activity
...
- **GitLab Data**: Repositories, configurations, databases
...
2. **Build Stage**: GitLab Runner builds application
3. **Test Stage**: Automated testing and quality checks
4. **Deploy Stage**: Deployment to Kubernetes cluster
